...
def doctor_dashboard_patient_info(_id):
    ...
    if (_id,) not in patients_id:
        flash("You don't have access to this patient!")
        return redirect(url_for('doctor_dashboard_patients'))
...
def doctor_dashboard_appointment_info(_id):
    ...
    consultas = cursor.fetchall()
    consultas_num = [consulta[0] for consulta in consultas]
    if _id not in consultas_num:
...
def doctor_dashboard_exam_info(_id):
    ...
    exams = cursor.fetchall()
    exams_codes = [exam[0] for exam in exams]
    if _id not in exams_codes:
        flash("You can't access this page!")
        return redirect(url_for('doctor_dashboard_exams'))
...
def doctor_dashboard_prescription_info(_id):
    ...
    prescriptions = cursor.fetchall()
    prescriptions_codes = [prescription[0] for prescription in prescriptions]
    if _id not in prescriptions_codes:
        flash("You can't access this page!")
